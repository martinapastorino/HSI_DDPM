...
from core.loaddata import AbuDataset, get_random_m

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('-c', '--config', type=str, default='config/PRISMA_256_DDPM.json',
    ...
    parser.add_argument('-val', '--val_freq', default=25000)
    ...
    train_path    = './dataset/unmixing_abundances/'
    train_set = AbuDataset(image_dir=train_path, unmix_met_list=opt['unmixing_methods'], augment=False)
    ...
    if opt['phase'] == 'train':
        while current_step < n_iter:
            ...
            for _, train_data in enumerate(train_loader):
                ...
                if current_step % 25000 == 0:
                    ...
                    for idx in range(10):
                        m_sample = get_random_m() # or substitute with a number between 0, ..., N-1 with N # of unmixing methods
                        diffusion.sample(m=m_sample, continous=False)
                        visuals = diffusion.get_current_visuals(sample=True)
                        sample_img = visual.tensor2img(
                            visuals['SAMPLE'])  # uint8
                        rgbsample_img = visual.tensor2rgb_band8(
                            visuals['SAMPLE'])  # uint8
        ...
        for idx in range(40):
            idx += 1
            m_sample = get_random_m()
            diffusion.sample(m=m_sample, continous=True)
            visuals = diffusion.get_current_visuals(sample=True)
